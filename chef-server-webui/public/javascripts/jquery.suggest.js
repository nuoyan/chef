	...
	(function($) {
		...
		$.suggest = function(input, options) {
			...
			options.buttonId.bind('click', function(){displayItems(options.list);})
			...
			function suggest() {
				...
				if (q.length >= options.minchars) {
					...
					for (item in options.list) {
						...
						if (q[0] === ">" || q[0] === "<" || q[0] === "=") {
							item_string = q[0].concat(" " + options.list[item]);
							if (q[0] !== "=" && q[1] === "=") {
								item_string = item_string[0] + q[1] + " " + item_string.substring(1, item_string.length).trim();
							}
						}
